...
- `anakins_dtls/_generated_hover_docs.py` is generated by
  `tools/generate_docs.py`; do not edit it manually.
- Treat `ruff check .` (run as part of `make`) as a test that must genuinely
  pass. Never bypass, weaken, or silence a lint failure instead of fixing the
  underlying code:
  - Do not add `# noqa` comments, inline `# ruff: noqa` directives, or
    per-file/per-rule ignores anywhere, including generated files.
  - Do not add rule exceptions or ignores to `pyproject.toml` or any ruff
    config to suppress a specific violation.
  - Do not restructure code merely to dodge a lint rule while keeping the
    underlying problem (e.g. renaming instead of removing unused code).
  - If a lint rule seems genuinely wrong for this project, stop and ask the
    user before changing lint configuration; do not decide unilaterally.
...
- Adds `noqa` suppressions or lint-config ignores instead of fixing the
  underlying lint violation.
